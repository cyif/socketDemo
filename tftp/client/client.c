...
int tftp_send_ack(struct tftp_conn *tc) {
    ...
    int result =sendto(tc->sock, pTftp_ack,TFTP_ACK_HDR_LEN, 0, (struct sockaddr*)(&(tc->peer_addr)), (*tc).addrlen);
    free(pTftp_ack);
    return result;
}
...
int tftp_send_data(struct tftp_conn *tc, int length) {
    ...
    //************Transmit new block**********************//
        ...
        if(!feof(tc->fp))
        {
            ...
            int result=sendto(tc->sock, pTftp_data, TFTP_DATA_HDR_LEN+dataLength, 0,(struct sockaddr*)(&(tc->peer_addr)), (*tc).addrlen);
